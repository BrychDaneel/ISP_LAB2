...
def add_stamp(path, dtime):
    ...
    if dtime is None:
        return path
    sec, msec = timestamp(dtime)
    fmt = "{path}_rmdt={posix_sec}_rmmsec={microsec}_"
    return fmt.format(path=path, timestamp=posix_sec, microsec=msec)
...
def get_versions_list(path):
    ...
    versions = [utils.splitstamp(f)[1] for f in files]
